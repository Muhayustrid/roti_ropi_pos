...
class TestMobilePOSLifecycle(IntegrationTestCase):
    ...
    def setUp(self) -> None:
        ...
        self.profile = make_valid_profile(
            f"Mobile POS E2E {frappe.generate_hash(length=8)}", self.cashier
        )
    ...
    def test_complete_lifecycle(self) -> None:
        ...
        frappe.local.form_dict = frappe._dict(
            {"pos_profile": self.profile.name, "value": self._batch_no}
        )
        ...
        # --- Phase 9: history ---
        history_result = sales_api.list(
            pos_profile=self.profile.name, status="all", q="", start=0, limit=20
        )
        ...
        return_name = return_result["data"]["sale"]["summary"]["name"]
