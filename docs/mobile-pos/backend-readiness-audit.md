...
- **Status: RESOLVED (P0-5, commit `feat: recover an unresolved closing without the client key`).** A new `POST v1.closing.recover` takes only `pos_profile` and no `X-Idempotency-Key`, because the lost key is the defect. Identity comes from the authenticated session, the authorized POS Profile, and the persisted `POS Opening Entry` / `POS Closing Entry` rows; `closing.py:execute_closing_recovery` locks the Opening, then adopts the Closing only when cashier, POS Profile, company, Opening reference, docstatus/status, and the stored `custom_mobile_pos_transaction_id` all agree. Any disagreement returns HTTP 409 `CLOSING_RECOVERY_REQUIRES_MANAGER` with a coarse, non-disclosing `reason` (`cashier_mismatch`, `profile_mismatch`, `opening_mismatch`, `closing_state_unrecoverable`, `closing_not_mobile_owned`); no unresolved Closing at all returns HTTP 409 `CLOSING_RECOVERY_NOT_AVAILABLE`. Recovery never creates a Closing Entry: it reuses the `v1.closing.submit` operation identity keyed by the Closing's own transaction id, so a rebuilt control row replays instead of repeating, a live lease still yields retryable `REQUEST_IN_PROGRESS`, and a Draft is submitted through the same `_resume_draft_closing` path the keyed submit uses. Separately, `sessions.py:unresolved_closing_request` now defines one blocking-row rule for both the projection and recovery, and a reservation that never reached a Closing Entry stops stranding the outlet once its lease expires. Evidence on `mobile-pos-regression.localhost`: `test_closing` `Ran 67 tests OK` (58 → 67), `test_authentication` allowlist set updated to 17 endpoints. RED before the fix: 8 `AttributeError: module 'roti_ropi_pos.api.v1.closing' has no attribute 'recover'` plus the dead-reservation projection failure. Mutations, each applied then reverted: dropping the cashier check and dropping the Opening check each submit another cashier's/another Opening's Closing (`Expected '_submit_persisted_closing' to not have been called. Called 1 times.`), dropping the expired-lease skip re-strands the outlet, and dropping the lease guard in `_claim_expired_request` lets recovery race a live request.
...
- **Status: RESOLVED (P0-6, commit `feat: map ERPNext document rejections into the v1 error envelope`).** Sale and return now persist through one shared `invoices.py:_persist_invoice`, which wraps `insert()`/`submit()` and maps a rejection class ERPNext itself declares into HTTP 422 `DOCUMENT_VALIDATION_FAILED` with `details.doctype`, `details.exception` (the class name Android routes on) and `details.display_message` (HTML-stripped, display only, never parsed). Because both directions call the same function, sale and return cannot disagree about the code for the same rejection. The mapped set is deliberately narrow: `_ERPNEXT_DOMAIN_REJECTIONS = (UOMMustBeIntegerError,)`. `UOMMustBeIntegerError` is the one declared class reachable through this API — the payload parser accepts a fractional qty and only ERPNext knows which UOMs are whole-number. `ProductBundleStockValidationError`, the POS Invoice module's only other declared class, stays unmapped because `_validate_total_stock` expands bundle components against the same availability ERPNext checks and already returns `INSUFFICIENT_STOCK` before insert. A bare `frappe.ValidationError` stays unmapped on purpose: ERPNext and the framework raise it both for rules this app already enforces and for a document this app built incorrectly, so mapping it would convert a server defect into a cashier-facing domain code. Every unmapped exception still reaches `responses.py:api_endpoint`, its request-ID logging, and Frappe's native HTTP 500. No ERPNext calculation, no core file, and no schema changed; the server remains authoritative for price, tax, grand total, payable, and refund amount. Evidence on `mobile-pos-regression.localhost`: `test_sale_task9` `Ran 60 tests OK` (58 → 60) and `test_return_task10` `Ran 23 tests OK` (21 → 23). RED before the fix: both new mapping tests raised `erpnext.utilities.transaction_base.UOMMustBeIntegerError: Row 1: Quantity (0.5) cannot be a fraction.` (`-0.5` on the return side) straight out of the endpoint. Mutations, each applied then reverted: adding `frappe.ValidationError` to the mapped tuple makes the mapping too broad and fails both unmapped-exception tests (`AssertionError: MandatoryError not raised`, and the return equivalent); removing `UOMMustBeIntegerError` from the tuple fails both mapping tests with the raw ERPNext traceback; returning `str(error)` instead of `strip_html_tags(str(error))` fails with `AssertionError: '<' unexpectedly found in "Row 1: Quantity (0.5) cannot be a fraction. To allow this, disable '<strong>Must be Whole Number</strong>' …"`; reverting the return path to bare `insert()`/`submit()` breaks sale/return symmetry and fails the return mapping test.
...
- **Status: RESOLVED (P0-7, commit `test: make POS fixtures own their preconditions on any four-app site`).** Evidence now comes from `selling-cutover.localhost`, which carries every app Roti depends on (`frappe, erpnext, payments, hrms, bakery_manufacturing, stock_additional, selling_additional, pos_direct_print, roti_ropi_pos`), so a missing-app failure can no longer be mistaken for coverage. All 15 Mobile POS modules run one at a time there, **370 tests, zero failures, zero errors**: `test_api_foundation` 17, `test_authentication` 36, `test_bootstrap` 9, `test_catalog` 19, `test_closing` 67, `test_customers` 7, `test_idempotency` 32, `test_mobile_pos_flow` 1, `test_opening_amounts` 21, `test_return_task10` 23, `test_sale_task9` 60, `test_sales` 28, `test_sessions` 10, `test_source_contracts` 38, `test_user_override` 2. The discovery failure is fixed at its root: `erpnext/tests/utils.py:2989` instantiates `BootStrapTestData()` at module import scope, and its `make_price_list` existence check includes `currency` against a hardcoded `"INR"`, so on an `IDR` site the check misses and the insert collides with `DuplicateEntryError: ('Price List', 'Standard Buying', ...)` — before any test runs. Roti now owns the 14-line `helpers.set_default_account_for_mode_of_payment` instead of importing an ERPNext test module, and `test_source_contracts.TestNoERPNextTestModuleImports` keeps the trap from returning. Three further fixture faults were measured and fixed in test code only: POS availability is `Bin.actual_qty` minus what submitted unconsolidated POS Invoices reserve, so it drifts downwards on a shared site (measured -590 here) and `INSUFFICIENT_STOCK` masked `INVALID_BATCH` / `INVALID_SERIAL_NUMBER` until `helpers.ensure_pos_availability` began measuring and topping up the shortfall; ERPNext's bootstrap ships `_Test Item` with `allow_negative_stock = 1`, which made `is_negative_stock_allowed` return True and silently disabled the oversell guard, so the sale suite now clears and restores the flag; and `validate_allow_to_set_serial_batch` refuses a Serial and Batch Bundle unless `Stock Settings.enable_serial_and_batch_no_for_item` is set (0 on this site), so the lifecycle suite saves, sets, and restores it and moves the Mode-of-Payment account setup ahead of `profile.save`. High-contention sale coverage is `test_sales.test_twenty_concurrent_attempts_create_one_invoice_and_request` and `test_idempotency.test_twenty_concurrent_attempts_create_one_opening_and_one_request`; validation-mapping coverage is the P0-6 `DOCUMENT_VALIDATION_FAILED` pair. Two site-state gaps on the test site were closed without touching any operational site: `frappe.utils.fixtures.sync_fixtures("roti_ropi_pos")` restored the owner-scoped Sales Invoice grant the site had missed since commit `9c0b7c7` (before/after snapshot diff: 5 rows added, 0 removed, 0 changed), and `throttle_user_limit: 5000` in that site's `site_config.json` stopped core's `throttle_user_creation` default of 60 from throttling the twenty-attempt test. No production file, no Frappe or ERPNext core file, and no schema changed; the server remains authoritative for price, tax, grand total, payable, return amount, and closing values.
